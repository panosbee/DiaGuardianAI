...
class EnsemblePredictor(BasePredictiveModel):
    ...
    Attributes:
        ...
        # the "stacking" strategy. (Placeholder for future
        # implementation)
    """
    def __init__(self, models: List[BasePredictiveModel],
        ...
        if self.strategy == "weighted_average":
            ...
            if not np.isclose(sum(weights), 1.0):
                raise ValueError(
                    "Weights for 'weighted_average' must sum to 1.0."
                )
            self.weights = np.array(weights)
        elif self.strategy not in ["average"]:  # Add other implemented strategies here
            raise NotImplementedError(
                f"Strategy '{strategy}' is not yet implemented. "
                f"Available: 'average', 'weighted_average'."
            )

        # Placeholder for stacking meta-learner
        # self.meta_learner = None
        # if self.strategy == "stacking":
        #     # self.meta_learner = SomeSklearnModel()  # e.g., LogisticRegression, Ridge, etc.
        #     # TODO: Initialize meta-learner for stacking strategy
        #     pass
    ...
    def train(self, X_train: Any, y_train: Any):
        ...
        # TODO: Implement training logic for stacking strategy if added.
        # if self.strategy == "stacking":
        #     print("Training meta-learner for stacking strategy...")
        #     # 1. Generate out-of-fold predictions from base models on X_train.
        #     #    This requires careful cross-validation to avoid data leakage.
        #     #    meta_features = self._get_oof_predictions(X_train, y_train)  # Helper method
        #     # 2. Train the meta_learner on these meta_features and y_train.
    ...
    def predict(self, X_current_state: Any) -> Dict[str, List[float]]:
        ...
        for model_idx, model in enumerate(self.models):
            try:
                ...
                if not isinstance(model_pred_dict, dict) or \
                   "mean" not in model_pred_dict or \
                   not isinstance(model_pred_dict["mean"], list) or \
                   not all(isinstance(p, (int, float)) for p in model_pred_dict["mean"]):
                    raise ValueError(
                ...
                all_mean_predictions_np.append(np.array(model_pred_dict["mean"], dtype=float))

                # Handle std_dev, optional for now or default to zeros if not present
                if "std_dev" in model_pred_dict and \
                   isinstance(model_pred_dict["std_dev"], list) and \
                   all(isinstance(s, (int, float)) for s in model_pred_dict["std_dev"]) and \
                   len(model_pred_dict["std_dev"]) == len(model_pred_dict["mean"]):
                    all_std_dev_predictions_np.append(np.array(model_pred_dict["std_dev"], dtype=float))
                else:
                    # If std_dev is missing or malformed, append zeros of the same length as mean
                    print(f"Warning: Model {model.__class__.__name__} (index {model_idx}) "
                          f"did not provide valid 'std_dev'. Using zeros.")
                    all_std_dev_predictions_np.append(np.zeros_like(all_mean_predictions_np[-1]))
        ...
        if not all_mean_predictions_np:
            print("Warning: No 'mean' predictions received from sub-models to ensemble.")
            return {"mean": [], "std_dev": []}

        # Ensure all prediction arrays have the same length
        first_pred_len = len(all_mean_predictions_np[0])
        if not all(len(p) == first_pred_len for p in all_mean_predictions_np) or \
           not all(len(s) == first_pred_len for s in all_std_dev_predictions_np):
        ...
        mean_predictions_array = np.stack(all_mean_predictions_np, axis=0) # Shape: (num_models, num_horizons)
        std_dev_predictions_array = np.stack(all_std_dev_predictions_np, axis=0) # Shape: (num_models, num_horizons)
        ...
        if self.strategy == "average":
            ...
            ensembled_std_dev_prediction = np.sqrt(np.mean(std_dev_predictions_array**2, axis=0)) # Root Mean Square of std_devs
            ...
            ensembled_std_dev_prediction = np.sqrt(np.average(
                std_dev_predictions_array**2, axis=0, weights=self.weights
            ))
        # elif self.strategy == "stacking":
        #     # Stacking would require a meta-learner to predict both mean and potentially std_dev,
        #     # or have its own UQ method.
        #     raise NotImplementedError("Stacking prediction strategy not fully implemented for mean and std_dev.")
        else:
            raise ValueError(f"Unknown or unsupported ensemble strategy: {self.strategy}")
    ...
    def save(self, path: str):
        ...
        """
        # TODO: Implement robust saving for the ensemble.
        # This might involve:
        # - Saving the ensemble's configuration (strategy, weights).
        # - Deciding on a strategy for sub-model persistence:
        #   - Save paths to independently saved sub-models.
        #   - Save sub-model configurations to allow re-instantiation.
        #   - Serialize sub-models if they are simple and picklable (less common for DL models).
        # - Saving the meta-learner if using stacking.
        print(
            f"Placeholder: Saving EnsemblePredictor to {path}. "
            f"(Detailed sub-model/meta-learner saving not implemented here)"
        )
        # Example using joblib for config:
        # import joblib
        # ensemble_config_data = {
        #     "strategy": self.strategy,
        #     "weights": self.weights.tolist() if self.weights is not None else None,
        #     "num_models": len(self.models),
        #     # Potentially add paths or configs for each sub-model if they
        #     # are to be reloaded
        #     "model_configs_or_paths": [  # Hypothetical
        #         model.get_config() or f"model_{i}_path.pth"
        #         for i, model in enumerate(self.models)
        #     ]
        # }
        # joblib.dump(ensemble_config_data, f"{path}_ensemble_config.joblib")
        # if self.strategy == "stacking" and self.meta_learner:
    ...
    def load(self, path: str):
        ...
        """
        # TODO: Implement robust loading for the ensemble.
        print(
            f"Placeholder: Loading EnsemblePredictor from {path}. "
            f"(Detailed sub-model/meta-learner loading not implemented here)"
        )
        # Example:
        # import joblib
        # ensemble_config_data = joblib.load(f"{path}_ensemble_config.joblib")
        # self.strategy = ensemble_config_data["strategy"]
        # self.weights = np.array(ensemble_config_data["weights"]) if ensemble_config_data["weights"] is not None else None
        # # Re-instantiating models would require stored configurations or a factory pattern.
        # # self.models = [ ... re-instantiate and load sub-models ... ]
        # if self.strategy == "stacking":
        #     try:
        #         self.meta_learner = joblib.load(f"{path}_meta_learner.joblib")
        #     except FileNotFoundError:
        #         print(
        #             f"Warning: Meta-learner file not found at "
        #             f"{path}_meta_learner.joblib for stacking strategy."
        #         )
        #         self.meta_learner = None  # Or raise error
        pass

if __name__ == '__main__':
    # Dummy models for testing ensemble
    class DummyModel(BasePredictiveModel):
        def __init__(self, model_id: str, prediction_value: float,
                     output_len: int = 3):
            super().__init__()
        ...
        def train(self, X_train, y_train): print(f"DummyModel {self.model_id} train called.")
        def predict(self, X_current_state) -> Dict[str, List[float]]: # Changed return type
        ...
        def save(self, path: str): print(f"DummyModel {self.model_id} save to {path}.")
        def load(self, path: str): print(f"DummyModel {self.model_id} load from {path}.")
    ...
    # Average ensemble
    avg_ensemble = EnsemblePredictor(models=[model1, model2, model3], strategy="average")
    avg_preds_dict = avg_ensemble.predict(dummy_X)
    print(f"Average Ensemble Predictions: Mean={avg_preds_dict['mean']}, StdDev={avg_preds_dict['std_dev']}")
    ...
    try:
        ...
        print(f"Weighted Ensemble Predictions: Mean={weighted_preds_dict['mean']}, StdDev={weighted_preds_dict['std_dev']}")
    except Exception as e:
