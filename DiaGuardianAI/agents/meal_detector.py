...
class MealDetector:
    ...
    def __init__(
        ...
        if self.detection_method == "ml_based":
            ...
            if not self.time_window_minutes > 0 or not self.min_samples_in_window > 0:
                raise ValueError(
    ...
    def detect_meal_event(
        self,
        cgm_history: List[float],
        timestamps: Optional[List[Any]] = None,
        other_features: Optional[Dict[str, Any]] = None,
    ) -> Tuple[
        float, Optional[Any], Optional[float]
    ]:  # Changed return type for size to float (carbs)
        ...
        if self.detection_method == "rule_based":
            ...
            if (
                features is None or len(features) == 0
            ):  # Should be handled by _extract_features_for_ml to return zeros
                print(
                    "MealDetector (ml_based): No features extracted. Returning no detection."
                )
                return 0.0, None, None
            ...
            if meal_probability > detection_threshold:
                est_time = (
                    timestamps[-1] if timestamps and len(timestamps) > 0 else None
                )
    ...
    def _detect_meal_rule_based(
        self, cgm_history: List[float], timestamps: Optional[List[Any]] = None
    ) -> Tuple[
        float, Optional[Any], Optional[float]
    ]:  # Changed return type for size to float (carbs)
        ...
        if timestamps and len(timestamps) == len(cgm_history):
            ...
            cgm_sample_rate_minutes = self.params.get(
                "cgm_sample_rate_minutes_for_rule", 5
            )
            samples_for_window = self.time_window_minutes // cgm_sample_rate_minutes
            samples_for_window = max(
                samples_for_window, self.min_samples_in_window
            )  # Ensure at least min_samples
            ...
            relevant_timestamps_segment = (
                timestamps[-len(relevant_history_segment) :]
                if timestamps and len(timestamps) >= len(relevant_history_segment)
                else [None] * len(relevant_history_segment)
            )

        if (
            len(relevant_history_segment) < 2
        ...
        if glucose_change >= self.rise_threshold:
            ...
            est_start_time = (
                relevant_timestamps_segment[0]
                if relevant_timestamps_segment
                and relevant_timestamps_segment[0] is not None
                else None
            )
            ...
            if (
                glucose_change > self.rise_threshold * 2.5
            ):  # e.g., >50mg/dL rise if threshold is 20
    ...
    def _extract_features_for_ml(
        ...
        """
        # Basic feature engineering for ML-based detection.
        # This can be extended with more advanced techniques such as
        # Fourier transforms or wavelet coefficients. Features from
        # `other_features` are appended when available.
        ...
        if len(cgm_array) >= 2:
            ...
            overall_slope = (cgm_array[-1] - cgm_array[0]) / max(
                1, len(cgm_array) - 1
            )  # Slope per interval
            ...
            if len(cgm_array) >= 4:  # Need at least 4 points for 3 intervals
                roc_last_3 = (
                    cgm_array[-1] - cgm_array[-4]
                ) / 3.0  # Avg change per interval
        ...
        if timestamps and len(timestamps) > 0:
            ...
            if hasattr(last_timestamp, "hour") and hasattr(
        ...
        # Current feature count so far: mean, std, min, max (4) + overall_slope,
        # roc_last_3, roc_last_2 (3) + time (4) = 11 features. Add any numeric
        # values from other_features if provided.

        if other_features:
            for val in other_features.values():
    ...
    def train_model(self, X_train: np.ndarray, y_train: np.ndarray):
        ...
        if self.detection_method == "ml_based":
            ...
            ):  # Simplified check: if model exists, assume it has fit
    ...
    def save_model(self, path: str):
        ...
        if self.detection_method == "ml_based" and self.ml_model:
            try:
                ...
                print(
                    f"MealDetector (ml_based): RandomForestClassifier model saved to {path}."
                )
            except Exception as e:
    ...
    def load_model(self, path: str):
        ...
        if self.detection_method == "ml_based":
            try:
                ...
                print(
                    f"MealDetector (ml_based): RandomForestClassifier model loaded successfully from {path}."
                )
            except FileNotFoundError:
                print(
                    f"MealDetector (ml_based): Error - Model file not found at {path}."
                )
...
if __name__ == "__main__":
    ...
    cgm_meal_start = [
        ...
        140.0,
        150.0,
    ]  # Rise of 50 in 25 mins (5 samples)
    ...
    print(
        f"Rule-based (No Meal): Prob={prob_nm:.2f}, Time={time_nm}, Carbs={carbs_nm}g"
    )
    ...
    features_meal = detector_ml_for_features._extract_features_for_ml(
        cgm_meal_start, timestamps_meal_start
    )
    print(
        f"Features for cgm_meal_start ({len(features_meal)} features): {np.round(features_meal, 4)}"
    )

    # Test with cgm_no_meal data
    features_no_meal = detector_ml_for_features._extract_features_for_ml(
        cgm_no_meal, timestamps_no_meal
    )
    print(
        f"Features for cgm_no_meal ({len(features_no_meal)} features): {np.round(features_no_meal, 4)}"
    )
    ...
    ]  # Ends at 12:05 Sunday
    features_short = detector_ml_for_features._extract_features_for_ml(
        cgm_short, timestamps_short
    )
    print(
        f"Features for cgm_short ({len(features_short)} features): {np.round(features_short, 4)}"
    )

    # Test with empty CGM history
    features_empty = detector_ml_for_features._extract_features_for_ml([], [])
    print(
        f"Features for empty CGM ({len(features_empty)} features): {np.round(features_empty, 4)}"
    )

    # Test without timestamps
    features_no_ts = detector_ml_for_features._extract_features_for_ml(
        cgm_meal_start, None
    )
    print(
        f"Features for cgm_meal_start (no timestamps) ({len(features_no_ts)} features): {np.round(features_no_ts, 4)}"
    )
    ...
    prob_ml_detect, time_ml_detect, carbs_ml_detect = (
        detector_ml_for_features.detect_meal_event(
            cgm_meal_start, timestamps_meal_start
        )
    )
    print(
        f"ML-based Detection (Meal Data): Prob={prob_ml_detect:.2f}, Time={time_ml_detect}, Carbs={carbs_ml_detect}g"
    )
